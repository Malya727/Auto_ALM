#!/usr/bin/env python3
"""
ALM Sync Script - DEV -> PROD
- Uses config.json for DEV/PROD model pairs
- Supports Revision Tag options: latest / create / select
- Checks workspace size and asks for sync confirmation
- Logs steps and shows summary table
"""
...
def list_revision_tags(token, model_id, workspace_id):
    ...
    resp.raise_for_status()
    return [t.get("name") for t in resp.json().get("revisions", [])]
...
# Sync Function
# --------------------------
def sync_revision_tag(token, dev_id, prod_id, tag_name, prod_ws, logger):
    headers = {"Authorization": f"AnaplanAuthToken {token}", "Content-Type": "application/json"}
...
def main():
    ...
    for idx, pair in enumerate(model_pairs, start=1):
        ...
        print(f"\nPair {idx}: DEV={dev_id} | PROD={prod_id}")
        print(f"Workspace IDs: DEV={dev_ws} | PROD={prod_ws}")
        ...
        print(f"Prod workspace: {bytes_to_human(used)}/{bytes_to_human(alloc)} ({pct:.2%})")

        ans = input("Sync this pair? (y/n) [y]: ").strip().lower() or "y"
        if ans != "y":
            summary_data.append([idx, dev_id, prod_id, tag_name, bytes_to_human(used), "-", "Skipped"])
            continue

        # Sync
        status = sync_revision_tag(token, dev_id, prod_id, tag_name, prod_ws, logger)
        summary_data.append([idx, dev_id, prod_id, tag_name, bytes_to_human(used), "-", status])
