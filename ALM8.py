#!/usr/bin/env python3
"""
Simple ALM Sync Script
- Username/password authentication
- Fetch workspace ID dynamically
- Create Revision Tag immediately
- Download Model History
- Estimate workspace size (MB/GB)
- Sync confirmed DEV->PROD pairs
- Logging
"""
...
# --------------------------
# Fetch Workspace ID using Model ID
# --------------------------
...
def run_export(token, workspace_id, model_id, export_id, out_dir, logger):
    ...
    r = requests.post(url, headers=headers, json={"localeName": "en_US"}, timeout=30)
    r.raise_for_status()
    task_data = r.json()
    task_id = None
    ...
    if not task_id:
        logger.info("Small model, exporting immediately without waiting for task.")
    else:
...
def main():
    ...
    for idx, pair in enumerate(model_pairs, start=1):
        ...
        print(f"\nProcessing pair #{idx}: DEV={dev_id}, PROD={prod_id}")
        ...
        # Create Revision Tag
        tag_name = input("Enter Revision Tag name (or leave blank for auto): ").strip() or f"AutoTag_{timestamp()}"
        try:
            create_revision_tag(token, dev_id, tag_name, dev_ws, logger)
        except Exception as e:
            logger.error(f"Revision Tag creation failed for DEV={dev_id}: {e}")
            continue
        ...
        if alloc and pct>0.95:
            ans = input("Workspace >95%. Continue sync? (y/n) [n]: ").strip().lower() or "n"
            if ans!="y":
