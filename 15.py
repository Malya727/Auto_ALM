...
# --------------------------
def find_export_id(token, workspace_id, model_id, export_name):
    """
    Get export action ID by name.
    Must include workspace in the path:
    GET /workspaces/{workspaceId}/models/{modelId}/exports
    """
    headers = {"Authorization": f"AnaplanAuthToken {token}"}
    url = f"{ANAPLAN_API_BASE}/workspaces/{workspace_id}/models/{model_id}/exports"
...
def run_export(token, workspace_id, model_id, export_id, out_dir, logger):
    """
    Run export and download file using the official v2 export sequence:
    - POST /exports/{exportId}/tasks
    - Poll /exports/{exportId}/tasks/{taskId}
    - GET /files
    - GET /files/{fileId}/chunks
    - GET /files/{fileId}/chunks/{chunkId}
    """
    headers = {"Authorization": f"AnaplanAuthToken {token}"}
    ...
    if not task_id:
        raise RuntimeError(f"Export task did not return taskId: {r.text}")
    ...
    # 3) Get list of files (export writes to a file in the model)
    files_url = f"{ANAPLAN_API_BASE}/workspaces/{workspace_id}/models/{model_id}/files"
    files_resp = requests.get(files_url, headers=headers, timeout=30)
    files_resp.raise_for_status()
    files = files_resp.json().get("files", [])

    # Heuristic: pick the latest read-only file with chunkCount > 0
    file_id = None
    if files:
        # Sort by lastModified if present; else just reverse
        try:
            files_sorted = sorted(
                files,
                key=lambda f: f.get("lastModified", ""),
                reverse=True
            )
        except Exception:
            files_sorted = list(reversed(files))
        for f in files_sorted:
            if f.get("chunkCount", 0) > 0 and f.get("isReadOnly"):
                file_id = f.get("id")
                break

    if not file_id:
        raise RuntimeError("Export file not found or has 0 chunks after task completion")

    # 4) Get list of chunks
    chunks_url = f"{ANAPLAN_API_BASE}/workspaces/{workspace_id}/models/{model_id}/files/{file_id}/chunks"
    chunks_resp = requests.get(chunks_url, headers=headers, timeout=30)
    chunks_resp.raise_for_status()
    chunks = chunks_resp.json().get("chunks", [])

    if not chunks:
        raise RuntimeError("No chunks returned for export file")

    # 5) Download chunks and write to file
    ensure_dir(out_dir)
    ...
    with open(fpath, "wb") as fh:
        for ch in chunks:
            ch_id = ch.get("id")
            ch_url = f"{ANAPLAN_API_BASE}/workspaces/{workspace_id}/models/{model_id}/files/{file_id}/chunks/{ch_id}"
            dl = requests.get(ch_url, headers=headers, stream=True, timeout=30)
            dl.raise_for_status()
...
def main():
    ...
    for idx, pair in enumerate(model_pairs, start=1):
        ...
        try:
            export_id = find_export_id(token, dev_ws, dev_id, export_name)
