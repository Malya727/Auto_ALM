#!/usr/bin/env python3
"""
Auto ALM Sync - Full Script
- Uses your working username/password auth
- Robust Revision Tag handling
- Table display, workspace size check, parallel sync
...
def run_export_and_download(token: str, workspace_id: str, model_id: str, export_id: str, out_dir: str, logger: logging.Logger):
    ...
    r.raise_for_status()
    task_id = r.json().get("task", {}).get("id") or r.json().get("taskId") or r.json().get("id")
    if not task_id: raise RuntimeError("Could not start export task")
    ...
    for _ in range(60):
        ...
        if status.lower() in ("completed","success"): break
...
# -------------------
# Revision Tag robust functions
# -------------------
def _try_post(url: str, headers: dict, payload: dict, logger, attempts=3, backoff=1):
    last_exc = None
...
def create_revision_tag(token: str, model_id: str, tag_name: str, logger: logging.Logger, workspace_id: Optional[str]=None):
    headers = {"Authorization": f"AnaplanAuthToken {token}", "Content-Type":"application/json"}
    payloads = [{"name": tag_name}, {"revision":{"name": tag_name}}]
    urls=[]
    if workspace_id: urls+=[f"{ANAPLAN_API_BASE}/workspaces/{workspace_id}/models/{model_id}/revisions"]
    urls += [f"{ANAPLAN_API_BASE}/models/{model_id}/revisions"]
    last_resp=None
    for url in urls:
        for payload in payloads:
            try:
                r = _try_post(url, headers, payload, logger)
                last_resp=r
                ...
                last_resp=e
    raise RuntimeError(f"Failed to create revision tag. Last response: {last_resp}")
...
def ask_revision_choice_for_pair(idx:int, dev_info:dict, prod_info:dict, token:str, export_action_name:str, logger, revision_file_path:str):
    print("\n"+"="*64)
    print(f"Pair #{idx+1}: {dev_info.get('model_name')} -> {prod_info.get('model_name')}")
...
def promote_revision_to_prod(task, token, logger):
    ...
    try:
        ...
        return {"dev":dev,"prod":prod,"status":r.status_code,"response":r.text}
    except Exception as e:
        logger.error(f"Promote error Prod={prod}: {e}")
        return {"dev":dev,"prod":prod,"status":"error","detail":str(e)}

def parallel_sync_executor(sync_tasks, token, logger):
...
def main():
    ...
    for idx,p in enumerate(pairs,1):
        ...
        rev_file=run_export_and_download(token, dev_info["workspace_id"], dev_info["model_id"], find_export_id_by_name(token, dev_info["model_id"], export_action_name, logger), MODEL_HISTORY_DIR, logger)
