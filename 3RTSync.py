...
def sync_revision_by_name(
    ...
    task_id = trigger_sync(token, prod_ws, prod_model, revision_id)
    print(f"✔ Sync Task Created. Task ID: {task_id}")

    print("📡 Polling Sync Status...")
    final_status = poll_sync(token, prod_ws, prod_model, task_id)
...
if __name__ == "__main__":
    sync_revision_by_name(
        username="email@example.com",
        password="yourPassword",
        rt_ws="RT_WORKSPACE_ID",
        rt_model="RT_MODEL_ID",
        prod_ws="PROD_WS_ID",
        prod_model="PROD_MODEL_ID",
        rt_revision_name="REVISION_TAG_NAME"
    )
