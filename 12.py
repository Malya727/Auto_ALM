...
def run_export(token, workspace_id, model_id, export_id, out_dir, logger):
    ...
    if task_id:
        ...
        for _ in range(60):  # increase wait time
            time.sleep(1)
...
def main():
    ...
    for idx, pair in enumerate(model_pairs, start=1):
        ...
        # Download Model History with retry
        rev_file = None
        for attempt in range(3):
            try:
                ...
                print(f"Attempt {attempt+1}/3: Export failed: {e}")
                if attempt == 2:
                    print("Skipping this pair after 3 failed attempts.")
        ...
        while True:
            try:
                ...
                if choice == "1":
                    ...
                    if not tags:
                        print("No RTs found. Using latest or create new.")
                        tag_name = get_latest_revision_tag(token, dev_id, dev_ws)
                    else:
                        print("Available RTs:")
                ...
                print(f"RT selection/creation failed: {e}")
                retry = input("Retry RT selection? (y/n): ").strip().lower()
                if retry != 'y':
                    tag_name = None
                    break
        ...
        try:
            ...
            print(f"❌ Sync request failed for DEV={dev_id} -> PROD={prod_id}")
