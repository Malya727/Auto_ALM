#!/usr/bin/env python3
"""
ALM Sync Script (Clean, Simple, User-Friendly)
"""
...
def get_workspace_id(token, model_id):
    ...
    resp.raise_for_status()
    ws_id = resp.json().get("workspace", {}).get("id")
    if not ws_id:
        raise RuntimeError(f"Cannot fetch workspace ID for model {model_id}")
    return ws_id
...
def main():
    ...
    for idx, pair in enumerate(model_pairs, start=1):
        ...
        try:
            ...
            print(f"Export ID found: {export_id}")
            rev_file = run_export(token, dev_ws, dev_id, export_id, MODEL_HISTORY_DIR, logger)
            print(f"Model history downloaded: {rev_file}")
